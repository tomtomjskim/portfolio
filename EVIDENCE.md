...
개인 실사용 프로젝트
= 비공개 코드와 개인 운영 기록을 바탕으로 공개 가능한 구조·화면·검증 범위만 설명
...
<a id="ev-idea-bank"></a>
### EV-IDEA-BANK — 아이디어 발굴·검토·기획 자동화

- **구분:** 비공개 개인 실사용 프로젝트
- **보여주는 내용:** 스케줄 기반 리서치·아이디어 생성, 사람의 채택·거절, 기획·설계 문서 생성과 LLM 출력 복구를 연결한 시스템
- **실제 사용:** 개인 운영 환경에 배포해 사용
- **공개 표현:** [아이디어 발굴·검토·기획 자동화](cases/idea-workflow-automation.md)
- **검증 범위:** 단위 테스트·TypeScript·빌드·품질 검사, 임시 DB 기반 Migration·Backfill·Rollback, 배포 전후 상태 확인
- **근거 강도:** 코드·운영 기록은 비공개이며, 공개 화면은 합성 데이터로 재현하므로 이 저장소만으로 독립 검증할 수 없음
- **비공개 범위:** 운영 도메인, 비공개 저장소, 인프라, 실제 아이디어·Prompt·Raw Response·비용·Session 데이터
- **증명하지 않는 것:** 외부 사용자·상용 SaaS·매출, 사업 타당성 자동 판정, 완전 자율 Venture Studio, production RAG·Multi-Agent 운영
