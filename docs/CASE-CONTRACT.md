# 사례 작성 기준

이 문서는 `cases/*.md`의 공통 구성과 공개 주장 범위를 정의합니다.

## 필수 구성

각 사례는 아래 순서를 유지합니다.

```text
핵심 질문
→ 한눈에 보기
→ 문제
→ 업무 환경과 제약
→ 확인 과정
→ 판단
→ 선택 기준과 대안
→ 구현
→ 검증과 실제 사용
→ 한계
→ 근거
```

면접 예상 질문, 답변 스크립트, 공격 질문, 리허설 메모는 공개 사례에 포함하지 않습니다. 해당 자료는 `personal-wiki`의 비공개 Interview SSOT에서 관리합니다.

## 15초 요약 기준

`한눈에 보기`에서는 다음 세 항목으로 사례를 빠르게 파악할 수 있어야 합니다.

| 항목 | 답해야 하는 질문 |
|---|---|
| 문제 | 무엇이 어려웠는가? |
| 판단 | 어떤 기준으로 무엇을 선택했는가? |
| 근거 | 무엇으로 확인할 수 있는가? |

요약 문장은 [portfolio-manifest.json](../portfolio-manifest.json)에서 관리하고, 사례 문서에는 같은 내용을 읽기 쉬운 표로 표시합니다. 검증 스크립트가 두 위치의 일치를 확인합니다.

## 사례 구분

| 구분 값 | 의미 |
|---|---|
| `sanitized-actual-work` | 실제 업무 경험을 공개 가능한 수준으로 비식별화 |
| `sanitized-actual-work-with-public-rnd-support` | 실제 업무 사례와 별도의 공개 연구·개발 근거를 함께 사용 |
| `private-personal-product` | 비공개 코드로 개인 운영한 제품을 합성 화면과 제한된 설명으로 공개 |
| `public-rnd` | 공개 코드·문서·CI로 확인 가능한 현재 개발 산출물 |

분류를 바꿔 과거 회사의 운영 소스, 개인 프로젝트와 공개 연구·개발 자료를 섞지 않습니다.

## 내용 작성 규칙

### 문제

- 사용자·운영자·개발자가 겪은 실제 문제를 설명
- 기술을 먼저 제시하지 않음
- 검증되지 않은 규모·성과 수치 금지

### 업무 환경과 제약

- 기존 환경과 변경 제약
- 공개할 수 없는 범위
- 기술·조직·운영 한계

### 확인 과정

- 어떤 질문으로 문제 범위를 좁혔는지
- 데이터·상태·권한·외부 시스템을 어떻게 확인했는지
- 원천 자료가 없는 세부 구현은 후보로 쓰지 않음

### 판단 / 선택 기준과 대안

- 선택한 방식
- 선택하지 않은 대안
- 비용·위험·적용 조건

### 구현

- TOM이 직접 다룬 범위
- 공개 가능한 구조와 책임
- 내부 테이블·endpoint·host·credential·원본 로그 금지

### 검증과 실제 사용

- 코드·DB·테스트·CI·사용자 흐름 중 실제 확인한 범위
- `not_run`, 수동 확인, 비공개 근거를 성공으로 둔갑시키지 않음
- PR·CI·테스트를 배포·운영 반영·외부 채택·성과 수치로 확대하지 않음
- 개인 실사용 프로젝트는 외부 사용자·상용 서비스와 구분

### 한계

- 책임·성숙도·공개 범위의 상한
- 사례 전체를 무력화하는 방어문이 아니라 해석 경계

### 근거

- [EVIDENCE.md](../EVIDENCE.md)의 안정된 근거 ID 참조
- 공개 저장소 또는 공개 가능한 사례 링크
- 비공개 원천 이름과 내부 경로 금지

## 합성 화면 규칙

`private-personal-product`의 화면은 실제 UI Source를 사용할 수 있지만 데이터는 합성 Fixture로 대체합니다.

필수:

- `합성 데이터로 재현한 화면` 표시
- 운영 Domain·private Repository·실데이터 없음
- 실제 Prompt·Raw Response·Token·비용·Session ID 없음
- Screenshot 원본의 내부정보 검수

합성 화면은 실제 외부 사용자·사용량·성과의 근거가 아닙니다.

## 공개 사례에 포함하지 않는 자료

- 면접 예상 질문과 답변 스크립트
- 공격 질문·방어 답변·리허설 상태
- 회사별 지원 문안과 Role Adapter
- Claim의 내부 상태표와 보류·제외 판단
- 개인 메모와 검수 대화
- 실제 운영 Domain과 비공개 저장소 경로
- 실제 Prompt·Raw Response·미공개 아이디어

## 새 주장 금지

사례 수정은 다음을 할 수 없습니다.

- 새로운 경력 사실 생성
- TOM의 역할을 원천보다 강화
- 측정하지 않은 성과 수치 생성
- 공개 연구·개발 자료를 이전 회사의 운영 업무로 변경
- 개인 프로젝트를 회사 실무나 외부 상용 서비스로 변경
- 계획·시제품·CI 통과를 완료 기능으로 표현

근거가 부족한 세부는 `현재 공개 근거로는 확인되지 않음` 또는 `추가 원천 확인 필요`로 남깁니다.